...
export const POST = async (req: NextRequest) => {
  try {
    ...
    // Check for the completed checkout session
    if (event.type === "checkout.session.completed") {
      const session = event.data.object;
      console.log("Session Data:", session);

      // Retrieve the complete session including line items and product metadata
      const retrieveSession = await stripe.checkout.sessions.retrieve(session.id, {
        expand: ["line_items.data.price.product"],
      });
      console.log("Retrieved Session Data:", retrieveSession);
      ...
      // Map order items from session line items
      const orderItems = lineItems?.map((item: any) => {
        return {
          product: item.price.product.metadata.productId,
          color: item.price.product.metadata.color || "N/A",
      ...
      // Save the customer with the new order
      await customer.save();
